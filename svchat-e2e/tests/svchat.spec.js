...
test('4. Отправка сообщения в групповой чат @realtime', async ({ context, page }) => {
...
test('5. Доставка второму пользователю в реальном времени (WebSocket) @realtime', async ({ browser }) => {
...
test('6. Превью последнего сообщения в списке чатов @realtime', async ({ context, page }) => {
...
test('7. Авто-переподключение после обрыва сети @realtime', async ({ context, page }) => {
...
test('9. Статус сообщения меняется на «прочитано» (✓✓) @realtime', async ({ browser }) => {
...
test('10. Личный чат показывает индикатор шифрования 🔒 @realtime', async ({ browser }) => {
...
test('11. Отправка файла — карточка с именем появляется @realtime', async ({ context, page }) => {
