...
const CLIENT_BUILD = 107 // номер актуальной клиентской сборки (index.html) для авто-обновления
...
const server = http.createServer(async (req, res) => {
  ...
  if (url === '/health') {
    ...
    const h = { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'no-cache, no-store, must-revalidate', 'X-Content-Type-Options': 'nosniff', 'Referrer-Policy': 'no-referrer', 'X-Frame-Options': 'SAMEORIGIN', 'Content-Security-Policy': "default-src 'self'; script-src 'self' 'sha256-GFCumoz46H3+zeOmTMu6RUzW6/lODMcGxg3HGmpUC6I=' 'sha256-Teo6bznhpC673bmFeNM+9sYI/kpWB9hnLsujc8XF8wo=' 'sha256-EPWGZOZfEBu49JDq/HQJ4LoLtGdLiVqUMs3AbSFQ+aY=' 'sha256-Q8eV7m/neHEf59aJ8eHIVM/H+ZFsZDZ60J1a4ikVEkQ=' 'sha256-RrJCSws2CH5usRS3o35JllpWHU18qUVj9FawGU7R+gg='; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; media-src 'self' data: blob:; connect-src 'self' wss: https:; font-src 'self' data:; worker-src 'self' blob:; frame-ancestors 'none'" }
...
server.listen(PORT, () => {
  console.log('SVchat server (v107: sync ver=CLIENT_BUILD, remove dead /contacts endpoint) на порту ' + PORT)
})
