...
const CLIENT_BUILD = 141 // номер актуальной клиентской сборки (index.html) для авто-обновления
...
server.listen(PORT, () => {
  console.log('SVchat server (v141: исправлен путь сокета /socket.io (была /engine.io — сокет не подключался)) на порту ' + PORT)
})
