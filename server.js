...
const CLIENT_BUILD = 142 // номер актуальной клиентской сборки (index.html) для авто-обновления
...
server.listen(PORT, () => {
  console.log('SVchat server (v142: создавать сокет до auth_account (был null при регистрации/входе) + путь /socket.io (была /engine.io — сокет не подключался)) на порту ' + PORT)
})
