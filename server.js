...
const CLIENT_BUILD = 171 // номер актуальной клиентской сборки (index.html) для авто-обновления
...
const dbReady = (async () => {
  ...
  try {
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS svchat_pins (user_id TEXT NOT NULL, room TEXT NOT NULL, PRIMARY KEY (user_id, room))`)
    const pinRows = await pool.query(`SELECT user_id, room FROM svchat_pins`)
    for (const r of pinRows.rows) { let s = userPins.get(r.user_id); if (!s) { s = new Set(); userPins.set(r.user_id, s) } s.add(r.room) }
...
async function dbSavePin(userId, room) {
  if (!pool) return
  try { await pool.query(`INSERT INTO svchat_pins (user_id, room) VALUES ($1,$2) ON CONFLICT DO NOTHING`, [userId, room]) } catch (e) { console.error('[db] pin:', e.message) }
}
async function dbDelPin(userId, room) {
  if (!pool) return
  try { await pool.query(`DELETE FROM svchat_pins WHERE user_id = $1 AND room = $2`, [userId, room]) } catch (e) { console.error('[db] pin del:', e.message) }
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (url === '/health') {
    ...
    res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'no-store', 'Content-Security-Policy': "default-src 'self'; script-src 'self' 'sha256-oGr/UMYpE2jkZNKLHFsS66+1CMECuf6gT/rY2Jhi2z4=' 'sha256-Teo6bznhpC673bmFeNM+9sYI/kpWB9hnLsujc8XF8wo=' 'sha256-EPWGZOZfEBu49JDq/HQJ4LoLtGdLiVqUMs3AbSFQ+aY=' 'sha256-O2f3zsK7kBCYSt0KF3+gEirrV/EXQXpmtibD5mWOeEA=' 'sha256-RrJCSws2CH5usRS3o35JllpWHU18qUVj9FawGU7R+gg='; style-src 'unsafe-inline'; connect-src 'self' wss: https: blob: data:" })
...
io.on('connection', (socket) => {
  ...
  // Закрепить/открепить чат (синхронизация между устройствами)
  socket.on('set_pin', (p = {}) => {
    const userId = String(p.userId || '').slice(0, 80)
    const token = p.auth ? String(p.auth).slice(0, 128) : ''
    if (!userId || !ownsUid(userId, token)) return
    if (!allow('rw', 40, 10000)) return
    const room = String(p.room || '').slice(0, 64)
    if (!room) return
    let s = userPins.get(userId); if (!s) { s = new Set(); userPins.set(userId, s) }
    if (p.pinned) { if (!s.has(room)) { s.add(room); dbSavePin(userId, room) } }
    else { if (s.has(room)) { s.delete(room); dbDelPin(userId, room) } }
  })
